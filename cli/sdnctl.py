    ...
    def cmd_bgp(self, args):
        ...
        if args.bgp_action == "status":
            ...
            if args.config_type == "bird":
                # Generate BIRD config without mutating the announcer's backend
                bgp = self.tm._bgp
                with bgp._lock:
                    config_path = bgp.config_dir / "bird.conf"
                    bgp.config_dir.mkdir(parents=True, exist_ok=True)
                    lines = [
                        "# BIRD configuration generated by SDN Controller",
                        f"# Timestamp: imported from {time.strftime('%Y-%m-%d %H:%M:%S')}",
                        f"# Announced routes: {len(bgp._announced)}",
                        "",
                    ...
                    if bgp._announced:
                        ...
                        for route in bgp._announced.values():
                    ...
                    lines.extend([
                        ...
                        f"  local as {bgp.local_as};",
                        f"  source address {bgp.router_id};",
                    ])
                    for peer in bgp._peers:
                        lines.append(f"  neighbor {peer.neighbor_ip} as {peer.remote_as};")
                    lines.extend([
                        "  ipv4 {",
                        "    import none;",
                    ...
                    with open(config_path, "w") as f:
                        f.write("\n".join(lines) + "\n")
                    print(f"BIRD config written to {config_path}")
